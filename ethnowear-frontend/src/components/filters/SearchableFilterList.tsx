...
import {
    Box, Checkbox, FormControlLabel, InputAdornment, Stack, TextField, Typography,
} from '@mui/material'
...
export default function SearchableFilterList({
    ...
    return (
        <Box sx={{ minWidth: 0 }}>
            {showTitle && (
                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: .5, textTransform: 'uppercase', fontWeight: 700 }}>
                    {title}
                </Typography>
            )}
            <TextField
            ...
            <Box sx={{
                ...
                {visibleItems.length === 0 ? (
                    <Typography variant="body2" color="text.secondary" sx={{ py: 1 }}>{t('filters.noMatchingOptions')}</Typography>
                ) : (
                    <Stack spacing={.25}>
                        {visibleItems.map(item => (
                            <FormControlLabel
                                ...
                                control={<Checkbox size="small" checked={selected.has(item.localName)} onChange={() => onToggle(item.localName)} sx={{ p: .75 }} />}
                                label={<Typography variant="body2" title={item.localName} sx={{ lineHeight: 1.3, textAlign: 'left', overflowWrap: 'anywhere' }}>{item.label || item.localName}</Typography>}
                                sx={{ width: '100%', m: 0, alignItems: 'center', '& .MuiFormControlLabel-label': { flex: 1, minWidth: 0 } }}
                            />
                        ))}
                    </Stack>
                )}
