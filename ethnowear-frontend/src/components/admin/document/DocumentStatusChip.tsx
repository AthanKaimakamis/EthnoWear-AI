...
type StatusKind = 'processing' | 'review' | 'transcription' | 'trust' | 'indexing' | 'job' | 'provenance'
...
const colors: Record<StatusKind, Record<string, ChipProps['color']>> = {
    ...
    transcription: { APPROVED: 'success', REJECTED: 'error', PENDING: 'warning', NOT_REQUIRED: 'default' },
...
export default function DocumentStatusChip({ kind, value, size = 'small' }: Props) {
    ...
    const key = kind === 'transcription' ? `documents.transcriptionStatus.${value}` : `documents.status.${kind}.${value}`
    return <Chip size={size} color={colors[kind][value] ?? 'default'} variant="outlined" label={t(key, { defaultValue: value })} />
}
