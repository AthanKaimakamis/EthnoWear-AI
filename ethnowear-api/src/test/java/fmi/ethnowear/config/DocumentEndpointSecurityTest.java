...
class DocumentEndpointSecurityTest {
    ...
    @Test
    void allowsReviewerFigureDecisionsButRejectsEditor() throws Exception {
        String endpoint = "/api/admin/document-pages/11/figures/21/approve";

        for (String role : List.of("ADMINISTRATOR", "REVIEWER")) {
    ...
    @Test
    void figureReviewPreviewRequiresAnAuthenticatedManagementRole() throws Exception {
        String endpoint = "/api/admin/document-pages/11/figures/21/content";
    ...
    @Test
    void reservesCorrectedTextResetForAdministrators() throws Exception {
        String endpoint = "/api/admin/document-pages/11/transcription/reset-from-current-ocr";
    ...
    @Test
    void allowsEveryManagementRoleToApplyAReviewedTextSuggestion() throws Exception {
        for (String endpoint : List.of(
                "/api/admin/document-pages/11/text-suggestions/51/apply",
                "/api/admin/document-pages/11/text-suggestions/51/issues/0/apply"
        )) {
            for (String role : List.of("ADMINISTRATOR", "REVIEWER", "EDITOR")) {
                mockMvc.perform(post(endpoint)
                                .header("Authorization", "Bearer " + validToken(role)))
    ...
    @Test
    void restrictsRawGroundedRetrievalToManagementRoles() throws Exception {
        String endpoint = "/api/admin/retrieval/search";
    ...
    @Test
    void restrictsProcessingMutationsToAdministratorsAndEditors() throws Exception {
    ...
    @Test
    void allowsArchiveEntryMutationsForAdministratorsAndEditors() throws Exception {
        for (String role : List.of("ADMINISTRATOR", "EDITOR")) {
            mockMvc.perform(post("/api/admin/archive-entries/security-check")
                            .header("Authorization", "Bearer " + validToken(role)))
                    .andExpect(status().isOk());
        }
    ...
    @Test
    void reservesDocumentDeletionForAdministrators() throws Exception {
    ...
    static class JwtSecurityTestConfiguration {
        ...
        AdminDocumentProcessingController processingController(
            ...
            return new AdminDocumentProcessingController(
    ...
    static class AdminSecurityController {
        ...
        @PostMapping(
                "/api/admin/document-pages/{pageId}/figures/{figureId}/approve"
        )
        String approveFigure() {
            return "ok";
        }

        @GetMapping(
                "/api/admin/document-pages/{pageId}/figures/{figureId}/content"
        )
        String previewFigure() {
            return "ok";
        }

        @PostMapping(
                "/api/admin/document-pages/{pageId}/transcription/reset-from-current-ocr"
        )
        String resetCorrectedText() {
            return "ok";
        }

        @PostMapping(
                "/api/admin/document-pages/{pageId}/text-suggestions/{suggestionId}/apply"
        )
        String applyTextSuggestion() {
            return "ok";
        }

        @PostMapping(
                "/api/admin/document-pages/{pageId}/text-suggestions/{suggestionId}/issues/{issueIndex}/apply"
        )
        String applyTextSuggestionIssue() {
            return "ok";
        }
        ...
        @GetMapping("/api/admin/processing/jobs")
        String processingJobs() {
            return "ok";
        }

        @PostMapping("/api/admin/processing/jobs/{jobId}/retry")
        String retryProcessingJob() {
            return "ok";
        }
        ...
        @PostMapping("/api/admin/retrieval/search")
        String retrieveApprovedEvidence() {
            return "ok";
        }
    }
