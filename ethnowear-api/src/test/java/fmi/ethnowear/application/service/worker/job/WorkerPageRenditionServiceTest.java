...
class WorkerPageRenditionServiceTest {
    ...
    @Test
    void targetedExtractionRejectsRenditionsForAnotherPage() {
        Document document = document(7L);
        DocumentPage target = page(document, 21L);
        DocumentPage other = page(document, 22L);
        DocumentProcessingJob job = activePageExtractionJob(111147L, document);
        job.setDocumentPage(target);

        assertThrows(
                WorkerRenditionConflictException.class,
                () -> service(job, other, Optional.empty()).upload(
                        111147L,
                        22L,
                        credentials(),
                        command(),
    ...
    private WorkerApiProperties propertiesWithMaximumSize(long bytes) {
        ...
        return new WorkerApiProperties(
                ...
                defaults.maximumOcrTextCharacters(),
                defaults.maximumOcrOutputSize()
        );
    }
