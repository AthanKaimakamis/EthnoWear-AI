...
public class MediaDeliveryService {
    ...
    public MediaDelivery findById(Long id) {
        ...
        return deliver(asset, id);
    }

    public MediaDelivery findPublicById(Long id) {
        requireId(id, "Media asset");

        MediaAsset asset = assetRepository
                .findPubliclyDeliverableById(id, FigureReviewState.APPROVED)
                .orElseThrow(() -> new ResourceNotFoundException("Media asset", id));

        return deliver(asset, id);
    }

    private MediaDelivery deliver(MediaAsset asset, Long id) {

        if (asset.getStorageState() == MediaStorageState.PURGED)
            throw new MediaContentPurgedException(id);
