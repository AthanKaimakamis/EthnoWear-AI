...
public class DocumentUploadValidator {
    ...
    public void validate(PdfDocumentUploadCommand command) {
        ...
        if(command.metadata().defaultSourceReferenceId() != null
                && command.metadata().sourceId() == null)
            throw new UnprocessableDocumentEvidenceException(
                    "A default source reference requires a document source"
            );

        if(command.metadata().defaultSourceReferenceId() != null
                && command.provenanceStatus() == ProvenanceStatus.UNKNOWN_SOURCE)
            throw new UnprocessableDocumentEvidenceException(
                    "Unknown provenance cannot use a default source reference"
            );
    }
