...
public class KnowledgeChunkService implements CrudService<KnowledgeChunkWriteDto, KnowledgeChunkDetails> {
    ...
    public KnowledgeChunkDetails update(Long id, KnowledgeChunkWriteDto input) {
        Document document = lockDocumentForChunk(id);
        KnowledgeChunk chunk = requireChunk(id);
        apply(chunk, input);
        KnowledgeChunk saved = chunkRepository.saveAndFlush(chunk);
        reconcile(document);
        return chunkMapper.toDetails(saved);
    }
    ...
    private Document lockDocumentForChunk(Long chunkId) {
        requireId(chunkId, "Knowledge chunk");

        return chunkRepository.findDocumentIdByChunkId(chunkId)
                .map(indexingStateReconciler::lockDocument)
                .orElse(null);
    }
