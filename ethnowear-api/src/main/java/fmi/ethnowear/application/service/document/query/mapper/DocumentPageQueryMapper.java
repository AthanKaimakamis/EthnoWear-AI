...
public class DocumentPageQueryMapper {
    ...
    public DocumentPageSummaryDetails toSummary(
        ...
        return new DocumentPageSummaryDetails(
                ...
                page.getCreatedAt(),
                page.getUpdatedAt(),
                new fmi.ethnowear.application.dto.document.query.DocumentPageRetirementDetails(
                        page.getRetiredAt() != null,
                        page.getRetiredAt(),
                        page.getRetiredBy(),
                        page.getRetirementReason()
                ),
                fmi.ethnowear.util.RowVersionUtils.token(page.getRowVersion())
        );
    }

    private DocumentPageQualitySummaryDetails quality(
            @NonNull DocumentPage page
    ) {
        if (page.getCurrentQualityAssessment() == null)
            return null;

        return new DocumentPageQualitySummaryDetails(
                page.getCurrentQualityAssessment().getId(),
                page.getCurrentQualityScore(),
                page.getCurrentQualityScore() == null
                        ? null
                        : page.getCurrentQualityScore()
                                .movePointRight(2)
                                .setScale(2, RoundingMode.HALF_UP),
                page.getCurrentQualityStatus(),
                page.getCurrentQualityPassedChecks() == null
                        ? 0
                        : page.getCurrentQualityPassedChecks(),
                page.getCurrentQualityFailedChecks() == null
                        ? 0
                        : page.getCurrentQualityFailedChecks(),
                page.getCurrentQualityAssessedAt()
        );
    }
