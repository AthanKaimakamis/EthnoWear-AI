...
public class ConceptCatalogService {
    ...
    private final ConceptCardEvidenceService evidenceService;
    ...
    public ConceptCatalogResultDetails search(ConceptCatalogQueryDto query, Pageable pageable) {
        ...
        Map<String, ConceptEvidenceSummaryDetails> evidence = evidenceService.summarize(
                query.entityType(),
                page.getContent().stream().map(EntityOntologyDetails::iri).toList()
        );

        List<EntityCardDetails> items = page.getContent()
                .stream()
                .map(entity -> cardMapper.toDetails(
                        entity,
                        evidence.getOrDefault(entity.iri(), ConceptEvidenceSummaryDetails.empty())
                ))
                .toList();
