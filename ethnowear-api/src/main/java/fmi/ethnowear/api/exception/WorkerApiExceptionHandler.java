...
public class WorkerApiExceptionHandler {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(WorkerApiExceptionHandler.class);

    @ExceptionHandler(RetrievalUnavailableException.class)
    public ResponseEntity<WorkerApiError> embeddingUnavailable(
    ...
    @ExceptionHandler(WorkerPreferredOcrInputConflictException.class)
    public ResponseEntity<WorkerApiError> preferredOcrInputConflict(
            @NonNull WorkerPreferredOcrInputConflictException ex
    ) {
        return error(
                HttpStatus.CONFLICT,
                "PREFERRED_OCR_INPUT_CONFLICT",
                ex.getMessage()
        );
    }

    @ExceptionHandler(WorkerOcrResultConflictException.class)
    public ResponseEntity<WorkerApiError> ocrResultConflict(
            @NonNull WorkerOcrResultConflictException ex
    ) {
        return error(
                HttpStatus.CONFLICT,
                "OCR_RESULT_CONFLICT",
    ...
    @ExceptionHandler(WorkerFigureConflictException.class)
    public ResponseEntity<WorkerApiError> figureConflict(
            @NonNull WorkerFigureConflictException ex
    ) {
        return error(
                HttpStatus.CONFLICT,
                "FIGURE_EXTRACTION_CONFLICT",
                ex.getMessage()
        );
    }

    @ExceptionHandler(WorkerQualityAssessmentConflictException.class)
    public ResponseEntity<WorkerApiError> qualityAssessmentConflict(
            @NonNull WorkerQualityAssessmentConflictException ex
    ) {
        return error(
                HttpStatus.CONFLICT,
                "QUALITY_ASSESSMENT_CONFLICT",
                ex.getMessage()
        );
    }

    @ExceptionHandler(WorkerVisionAssessmentConflictException.class)
    public ResponseEntity<WorkerApiError> visionAssessmentConflict(
            @NonNull WorkerVisionAssessmentConflictException ex
    ) {
        return error(
                HttpStatus.CONFLICT,
                "VISION_ASSESSMENT_CONFLICT",
                ex.getMessage()
        );
    }

    @ExceptionHandler(WorkerIndexResultConflictException.class)
    public ResponseEntity<WorkerApiError> indexResultConflict(
            @NonNull WorkerIndexResultConflictException ex
    ) {
        return error(
                HttpStatus.CONFLICT,
                "INDEX_RESULT_CONFLICT",
                ex.getMessage()
        );
    }
    ...
    @ExceptionHandler(WorkerVisionValidationException.class)
    public ResponseEntity<WorkerApiError> invalidVisionResult(
            @NonNull WorkerVisionValidationException ex
    ) {
        return error(
    ...
    public ResponseEntity<WorkerApiError> unexpected(Exception ex) {
        LOGGER.error("Unexpected internal worker API error", ex);
