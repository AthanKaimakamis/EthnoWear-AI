...
public class ArchiveApiExceptionHandler extends BaseExceptionHandler {
    ...
    @ExceptionHandler(MediaContentPurgedException.class)
    public ResponseEntity<Map<String, Object>> purged(
            @NonNull MediaContentPurgedException ex
    ) {
        return error(
                HttpStatus.GONE,
                "MEDIA_CONTENT_PURGED",
                ex.getMessage()
        );
    }
