...
from ethnowear_document_worker.pdf.inspector import PdfInspectionError, inspect_pdf
...
def test_inspector_prediction_matches_pymupdf_fractional_page_size(
    tmp_path: Path,
) -> None:
    pdf_path = tmp_path / "input.pdf"
    create_pdf(pdf_path, [(515.760009765625, 728.4000244140625)])

    result = inspect_pdf(pdf_path, 10, 300, 20_000, 20_000, 100_000_000)

    document = pymupdf.open(pdf_path)
    try:
        pixmap = document[0].get_pixmap(dpi=300)
        assert (result.pages[0].pixel_width, result.pages[0].pixel_height) == (
            pixmap.width,
            pixmap.height,
        )
    finally:
