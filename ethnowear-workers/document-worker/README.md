...
```shell
.venv/bin/python -m ethnowear_document_worker
```
...
| `ETHNOWEAR_WORKER_TEMPORARY_ROOT` | `/tmp/ethnowear-document-worker` | Absolute temporary workspace root |
...
`python -m ethnowear_document_worker.health` exits successfully only while runtime
