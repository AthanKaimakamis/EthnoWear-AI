...
async def run_worker(settings: QualityWorkerSettings) -> None:
    mark_not_ready(settings.temporary_root)
    removed = cleanup_stale_workspaces(settings.temporary_root)
    if removed:
        logger.info("stale_workspaces_removed", extra={"removed_count": removed})
    ...
    try:
        async with httpx.AsyncClient(
            ...
            api_client = QualityWorkerApiClient(settings, http_client)
            ...
            processor = QualityAssessmentProcessor(
                ...
                dictionary=BulgarianDictionary.load(
                    settings.bulgarian_dictionary_path
                ),
            )
            runner = WorkerRunner(
                poller=JobPoller(
