...
class VisionWorkerApiClient(WorkerApiTransport):
    ...
    async def get_assessment_context(
        ...
        response = await self._http_client.get(
            self._url(
                self._job_path(credentials.job_id, "vision-assessment/context")
            ),
            headers=self._claim_headers(credentials.claim_token),
        )
        self._require_status(response, 200)
        result = self._parse_response(
            response,
            VisionAssessmentContext,
            "vision assessment context",
        )

        if result.job_id != credentials.job_id:
            raise WorkerApiContractError(
                "Vision assessment context identifies a different job"
            )
    ...
    async def submit_assessment(
        self,
        credentials: ClaimCredentials,
        assessment: VisionAssessmentRequest,
    ) -> VisionAssessmentResponse:
        response = await self._http_client.post(
            self._url(self._job_path(credentials.job_id, "vision-assessment")),
            headers=self._claim_headers(credentials.claim_token),
        ...
        if result.job_id != credentials.job_id:
            raise WorkerApiContractError(
                "Vision assessment response identifies a different job"
            )
